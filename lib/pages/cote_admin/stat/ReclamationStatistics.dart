...
class ReclamationStatistics extends StatelessWidget {
  const ReclamationStatistics({Key? key}) : super(key: key);

  Future<int> _fetchReclamationCount() async {
    try {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<int>(
      future: _fetchReclamationCount(),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return Center(
            child: SpinKitCircle(
              color: Colors.pink, // Couleur de l'indicateur de chargement
          ...
          int reclamationCount = snapshot.data ?? 0;
          return Card(
            elevation: 3,
